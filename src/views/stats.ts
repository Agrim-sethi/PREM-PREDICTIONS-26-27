...
function computeStats(): Record<Player, PlayerStats> {
  ...
  for (let gw = 1; gw <= 38; gw++) {
    ...
    PLAYERS.forEach(p => {
      ...
      // Everything in finalPoints beyond rawPoints is attributable to cards.
      // Nemesis is tracked separately below, so the card-aided figure here is
      // the non-Nemesis uplift from Captain/Wild/Chaos/Floor, plus Mirror's
      // impact where applicable through the effective prediction scores.
  ...
  PLAYERS.forEach(p => {
    // finalPoints here is pre-Nemesis; apply the tracked Nemesis movement so
    // this matches the season leaderboard's actual points.
...
export function renderStats(): string {
  ...
  const completedGameweeks = new Set(
    Array.from({ length: 38 }, (_, i) => i + 1).filter(gw => {
      const matches = store.getMatchesByGW(gw);
  ...
  return `<div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:10px;margin-bottom:18px;">
      ...
      ${statCard('Best Accuracy', rows.slice().sort((a,b) => ((stats[b].correctOutcomes / Math.max(1,stats[b].predictionsMade)) - (stats[a].correctOutcomes / Math.max(1,stats[a].predictionsMade))))[0] ?? '—', rows[0] ? pct(stats[rows[0]].correctOutcomes, stats[rows[0]].predictionsMade) : '')}
      ${statCard('Most Exact Scores', rows.slice().sort((a,b) => stats[b].exactScores - stats[a].exactScores)[0] ?? '—', rows.length ? `${stats[rows[0]].exactScores} exact` : '')}
      ${statCard('Most Card Impact', rows.slice().sort((a,b) => stats[b].cardAidedPoints - stats[a].cardAidedPoints)[0] ?? '—', rows.length ? `${stats[rows[0]].cardAidedPoints >= 0 ? '+' : ''}${stats[rows[0]].cardAidedPoints} pts` : '')}
    </div>
      ...
      <div class="table-scroll" tabindex="0" aria-label="Participant statistics table">
        <table style="width:100%;border-collapse:collapse;text-align:left;font-size:12px;min-width:1220px;">
          <thead><tr style="border-bottom:1px solid var(--line);color:var(--muted);font:11px 'JetBrains Mono',monospace;">
            <th style="padding:8px 5px;">Player</th>
            <th style="padding:8px 5px;">Preds Made</th>
            <th style="padding:8px 5px;">Correct Outcome</th>
            <th style="padding:8px 5px;">Outcome %</th>
            <th style="padding:8px 5px;">Exact Scores</th>
            <th style="padding:8px 5px;">Exact %</th>
            <th style="padding:8px 5px;">Unique Pts</th>
            <th style="padding:8px 5px;">Unique %</th>
            <th style="padding:8px 5px;">Raw Pts</th>
            <th style="padding:8px 5px;">Card Aided</th>
            <th style="padding:8px 5px;">Nemesis Net</th>
            <th style="padding:8px 5px;">Final Pts</th>
          </tr></thead>
          <tbody>${rows.map(p => {
            const s = stats[p];
    ...
    <div class="panel-box" style="background:var(--panel-2);border:1px solid var(--line);border-radius:6px;padding:16px;margin-top:16px;overflow-x:auto;">
      ...
      <div class="table-scroll" tabindex="0" aria-label="Card usage table">
        <table style="width:100%;border-collapse:collapse;text-align:left;font-size:12px;min-width:900px;">
          <thead><tr style="border-bottom:1px solid var(--line);color:var(--muted);font:11px 'JetBrains Mono',monospace;">
            <th style="padding:8px 5px;">Player</th>
            <th style="padding:8px 5px;">Total Cards</th>
            <th style="padding:8px 5px;">Captain</th>
            <th style="padding:8px 5px;">Wild</th>
            <th style="padding:8px 5px;">Chaos</th>
            <th style="padding:8px 5px;">Floor</th>
            <th style="padding:8px 5px;">Mirror</th>
            <th style="padding:8px 5px;">Nemesis</th>
            <th style="padding:8px 5px;">Card Uplift</th>
          </tr></thead>
          <tbody>${rows.map(p => {
            const s = stats[p];
            return `<tr style="border-bottom:1px solid var(--line);">
              <td style="padding:10px 5px;font-family:'Oswald',sans-serif;font-weight:bold;">${p}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;">${s.cardsPlayed}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;">${s.captainUsed}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;">${s.wildcardUsed}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;">${s.chaosUsed}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;">${s.floorUsed}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;">${s.mirrorUsed}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;">${s.nemesisUsed}</td>
              <td style="padding:10px 5px;font-family:'JetBrains Mono',monospace;color:var(--pitch);">${s.cardAidedPoints >= 0 ? '+' : ''}${s.cardAidedPoints}</td>
            </tr>`;
          }).join('')}</tbody>
        </table>
      </div>
    ...
    <div style="margin-top:12px;color:var(--muted);font-size:10px;font-family:'JetBrains Mono',monospace;line-height:1.5;">
      OUTCOME % = correct outcome predictions ÷ predictions made · EXACT % = exact scorelines ÷ predictions made · UNIQUE % = unique points ÷ predictions made. Raw points exclude card multipliers/effects. Card Aided = final points − raw points, with Nemesis shown separately.
    </div>
  </div>`;
}
