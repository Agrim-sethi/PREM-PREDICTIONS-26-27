...
function readEditableInputs(): Record<string, Prediction> {
  ...
  document.querySelectorAll('.pred-input').forEach(input => {
    ...
    if (!(admin || player === profile?.player)) return;
    ...
    if (i.value !== '') {
      ...
      if (!Number.isNaN(value)) {
        ...
        if (team === 'away') preds[key].away = value;
      }
    }
  });
  return preds;
...
export function renderGameweek(): string {
  ...
  if (matches.length === 0) {
    const hint = available.length ? `Fixtures currently exist for: ${available.map(g => `GW${g}`).join(', ')}.` : 'No fixtures have been imported yet.';
    return html + `<div class="panel-box" style="padding:20px;color:var(--muted);text-align:center;">No fixtures set up for GW${gw}. ${admin ? 'Open Fixtures Setup and click “Import full season fixtures”, or add matches manually.' : 'Ask the administrator to import the season fixtures.'}<div style="margin-top:10px;font-size:12px;font-family:'JetBrains Mono',monospace;">${hint}</div></div>`;
  }
    ...
    html += `<div class="panel-box" style="background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:16px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px;">
        ${PLAYERS.map(p => {
          ...
          const hasDraft = draftPredictions.has(predictionKey(m.id, p));
          return `<div style="background:var(--ink);padding:10px;border-radius:4px;border:1px solid ${p === player ? 'var(--pitch)' : (isMirrored ? 'var(--violet)' : 'var(--line)')};">
            <div style="display:flex;justify-content:space-between;align-items:center;font-size:12px;color:${p === player ? 'var(--pitch)' : 'var(--muted)'};margin-bottom:6px;font-weight:bold;gap:6px;"><span>${p}${p === player && !admin ? ' · YOU' : ''}</span>${editable ? `<span style="font-size:9px;color:var(--pitch);font-family:JetBrains Mono,monospace;">${hasDraft ? 'EDIT · DRAFT' : 'EDIT'}</span>` : '<span style="font-size:9px;color:var(--muted);font-family:JetBrains Mono,monospace;">VIEW</span>'}</div>
            ...
            <div style="display:flex;gap:4px;align-items:center;"><input type="number" class="pred-input" data-match="${m.id}" data-player="${p}" data-team="home" value="${editable && original && original.home !== null ? original.home : (shownHome !== null ? shownHome : '')}" placeholder="H" ${editable ? '' : 'readonly'} style="width:48px;background:var(--panel);border:1px solid var(--line);color:var(--chalk);padding:5px;text-align:center;${editable ? '' : 'opacity:.65;cursor:not-allowed;'}"><span>-</span><input type="number" class="pred-input" data-match="${m.id}" data-player="${p}" data-team="away" value="${editable && original && original.away !== null ? original.away : (shownAway !== null ? shownAway : '')}" placeholder="A" ${editable ? '' : 'readonly'} style="width:48px;background:var(--panel);border:1px solid var(--line);color:var(--chalk);padding:5px;text-align:center;${editable ? '' : 'opacity:.65;cursor:not-allowed;'}"></div>
  ...
  html += `</div>`;
  html += `<div style="margin-top:20px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;"><button id="save-all-preds" style="background:var(--pitch);color:#0d1712;padding:10px 20px;border:none;border-radius:4px;font-family:'Oswald',sans-serif;font-size:14px;font-weight:bold;cursor:pointer;">SAVE ${admin ? 'ALL' : 'MY'} PREDICTIONS FOR GW${gw}</button><span style="font-size:11px;color:var(--muted);font-family:'JetBrains Mono',monospace;">Auto-saves every 3 seconds</span></div>`;

  const scores = calculateGameweekScores(gw, matches, store.state.predictions, gwCards);
  applyNemesisSteals(scores, gwCards);
  html += `<div class="panel-box" style="background:var(--panel-2);border:1px solid var(--line);border-radius:6px;padding:16px;margin-top:24px;"><h3 style="font-family:'Oswald',sans-serif;margin-bottom:12px;">GW${gw} Points Breakdown</h3><table style="width:100%;border-collapse:collapse;text-align:left;font-size:13px;"><thead><tr style="border-bottom:1px solid var(--line);color:var(--muted);font-family:'JetBrains Mono',monospace;"><th style="padding:8px 4px;">Player</th><th style="padding:8px 4px;">Raw Pts</th><th style="padding:8px 4px;">Cards Applied</th><th style="padding:8px 4px;">Final GW Pts</th></tr></thead><tbody>${PLAYERS.map(p => { const sc = scores[p]; const pCards = gwCards.filter(c => c.player === p || c.target === p || c.card === 'chaos'); const cardTags = pCards.map(c => c.card === 'chaos' ? `<span style="color:var(--red)">CHAOS</span>` : c.card === 'mirror' ? `<span style="color:var(--violet)">MIRROR ${c.player === p ? 'PLAYED' : `BY ${c.player}`}</span>` : c.card === 'nemesis' ? `<span style="color:var(--maroon)">NEMESIS ${c.player === p ? `→ ${c.target}` : `BY ${c.player}`}</span>` : c.player === p ? `<span style="color:var(--pitch)">${c.card.toUpperCase()}</span>` : `<span style="color:var(--maroon)">Target of ${c.card}</span>`).join(', '); return `<tr style="border-bottom:1px solid var(--line);"><td style="padding:8px 4px;font-weight:bold;">${p}</td><td style="padding:8px 4px;font-family:'JetBrains Mono',monospace;">${sc.rawPoints}</td><td style="padding:8px 4px;font-size:11px;">${cardTags || '-'}</td><td style="padding:8px 4px;font-weight:bold;font-family:'JetBrains Mono',monospace;color:var(--pitch);">${sc.finalPoints}</td></tr>`; }).join('')}</tbody></table></div>`;
  return html;
}
