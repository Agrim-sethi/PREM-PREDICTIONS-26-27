...
export function renderGameweek(): string {
  ...
  html += `<div class="gameweek-actions" style="margin-top:20px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
  ...
  html += `<div class="panel-box" style="background:var(--panel-2);border:1px solid var(--line);border-radius:6px;padding:16px;margin-top:24px;"><h3 style="font-family:'Oswald',sans-serif;margin-bottom:12px;">GW${gw} Points Breakdown</h3><div class="table-scroll" tabindex="0" aria-label="Gameweek points breakdown"><table style="width:100%;border-collapse:collapse;text-align:left;font-size:13px;min-width:460px;"><thead><tr style="border-bottom:1px solid var(--line);color:var(--muted);font-family:'JetBrains Mono',monospace;"><th style="padding:8px 4px;">Player</th><th style="padding:8px 4px;">Raw Pts</th><th style="padding:8px 4px;">Cards Applied</th><th style="padding:8px 4px;">Final GW Pts</th></tr></thead><tbody>${PLAYERS.map(p => { const sc = scores[p]; const ownCards = gwCards.filter(c => c.player === p); const targetCards = gwCards.filter(c => c.target === p && c.player !== p); const chaosCards = gwCards.filter(c => c.card === 'chaos'); const tags = [...ownCards, ...targetCards, ...chaosCards].map(c => {
    if (c.card === 'chaos') return `<span style="color:var(--red)">CHAOS APPLIED</span>`;
    if (c.player === p) {
      if (c.card === 'mirror') return `<span style="color:var(--violet)">MIRROR PLAYED</span>`;
      if (c.card === 'nemesis') return `<span style="color:var(--maroon)">NEMESIS PLAYED${c.target ? ` → ${c.target}` : ''}</span>`;
      return `<span style="color:var(--pitch)">${c.card.toUpperCase()} PLAYED</span>`;
    }
    if (c.card === 'mirror') return `<span style="color:var(--violet)">TARGETED BY MIRROR · ${c.player}</span>`;
    if (c.card === 'nemesis') return `<span style="color:var(--maroon)">TARGETED BY NEMESIS · ${c.player}</span>`;
    return `<span style="color:var(--muted)">TARGETED BY ${c.card.toUpperCase()} · ${c.player}</span>`;
  }).join(', '); return `<tr style="border-bottom:1px solid var(--line);"><td style="padding:8px 4px;font-weight:bold;">${p}</td><td style="padding:8px 4px;font-family:'JetBrains Mono',monospace;">${sc.rawPoints}</td><td style="padding:8px 4px;font-size:11px;">${tags || '-'}</td><td style="padding:8px 4px;font-weight:bold;font-family:'JetBrains Mono',monospace;color:var(--pitch);">${sc.finalPoints}</td></tr>`; }).join('')}</tbody></table></div></div>`;
  return html;
}
