...
function captainUsedInGW(player: string, gw: number): boolean {
  return store.state.cards.some(c => c.player === player && c.card === 'captain' && c.gw === gw);
}

function remainingLabel(player: string, card: CardType, gw: number): string {
  const def = CARDS[card];
  if (def.perGameweek) return captainUsedInGW(player, gw) ? 'USED THIS GW' : '1 THIS GW';
  return `${Math.max(0, def.allowance - playerCardCount(player, card))} LEFT`;
}
...
function cardPlayForm(player: string, admin = false): string {
  ...
  const cards = Object.entries(CARDS).map(([key, def]) => {
    ...
    const usedThisGW = def.perGameweek && captainUsedInGW(player, defaultGW);
    const exhausted = !def.perGameweek && playerCardCount(player, card) >= def.allowance;
    const disabled = usedThisGW || exhausted;
    const suffix = def.perGameweek ? (usedThisGW ? ' · USED THIS GW' : ' · 1 THIS GW') : ` · ${Math.max(0, def.allowance - playerCardCount(player, card))} LEFT`;
    return `<option value="${card}" ${disabled ? 'disabled' : ''}>${def.label}${suffix}</option>`;
  }).join('');
  ...
  return `<div class="panel-box" style="background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:16px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;">
      <div><label>Gameweek</label><input type="number" id="fc-gw" min="1" max="38" value="${defaultGW}"></div>
...
export function renderCardLog(): string {
  ...
  if (!admin && currentPlayer) {
    ...
    Object.entries(CARDS).forEach(([key, def]) => {
      ...
      const usedThisGW = def.perGameweek ? captainUsedInGW(currentPlayer, 1) : false;
      const remaining = def.perGameweek ? (usedThisGW ? 0 : 1) : Math.max(0, def.allowance - used);
      html += `<div style="border:1px solid ${def.color}55;background:var(--ink);border-radius:5px;padding:10px 12px;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;">
          ...
          <span style="font:700 11px 'JetBrains Mono',monospace;color:${remaining === 0 ? 'var(--red)' : 'var(--pitch)'};">${def.perGameweek ? (remaining === 1 ? '1 THIS GW' : 'USED THIS GW') : `${remaining} LEFT`}</span>
        </div>
...
export function attachCardLogHandlers(reRender: () => void) {
  ...
  document.querySelectorAll('.del-card-btn').forEach(btn => btn.addEventListener('click', async e => {
    ...
    if (!id) return;
    if (window.confirm('Delete this card play? The card becomes available again.')) {
  ...
  const playerInput = document.getElementById('fc-player') as HTMLInputElement | HTMLSelectElement | null;

  const refreshCardOptions = () => {
    if (!gwInput || !cardSelect || !playerInput) return;
    const gw = parseInt(gwInput.value, 10) || 1;
    const player = admin ? (playerInput as HTMLSelectElement).value : profile?.player;
    if (!player) return;
    [...cardSelect.options].forEach(option => {
      const card = option.value as CardType;
      const def = CARDS[card];
      const usedThisGW = def.perGameweek && captainUsedInGW(player, gw);
      const exhausted = !def.perGameweek && playerCardCount(player, card) >= def.allowance;
      option.disabled = usedThisGW || exhausted;
      option.textContent = def.perGameweek
        ? `${def.label} · ${usedThisGW ? 'USED THIS GW' : '1 THIS GW'}`
        : `${def.label} · ${Math.max(0, def.allowance - playerCardCount(player, card))} LEFT`;
    });
  };

  gwInput?.addEventListener('change', refreshCardOptions);
  playerInput?.addEventListener('change', refreshCardOptions);
  refreshCardOptions();
  ...
  document.getElementById('fc-submit')?.addEventListener('click', async () => {
    ...
    if (card === 'captain' && captainUsedInGW(player, gw)) {
      warn.textContent = `${player} has already played a Captain in GW${gw}. Only 1 Captain is allowed per player per gameweek.`;
      return;
    }

    try {
      await store.addCard({ id: genId(), gw, player, card, matchNo, target: target as any, note, ts: Date.now() });
      reRender();
    } catch (err) {
      warn.textContent = err instanceof Error ? err.message : 'Card could not be saved. Check your permissions.';
    }
  });
}
