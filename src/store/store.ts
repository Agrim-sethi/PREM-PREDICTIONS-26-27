...
function normalizeLoadedCard(raw: any, docId: string): CardEntry | null {
  ...
  return {
    id: docId,
    slotId: typeof raw.slotId === 'string' && raw.slotId ? raw.slotId : docId,
    gw,
    player,
    card,
...
export const store = {
  ...
  async setGameweekLocked(gw: number, locked: boolean): Promise<void> {
    ...
    try {
      await setDoc(doc(matchesCol, lockDocId(gw)), {
        kind: 'gameweekLock',
        gw,
        locked,
        updatedAt: Date.now()
      ...
      throw firestoreError(`GW${gw} lock could not be updated`, error);
    }
  },
  ...
  getCardRemaining(player: Player, card: CardType, gw: number): number {
    if (CARDS[card].perGameweek) return slotIsUsed(this.state.cards, cardSlotId(player, card, gw)) ? 0 : 1;
  ...
  async addCard(card: CardEntry): Promise<void> {
    ...
    try {
      await runTransaction(db, async tx => {
        ...
        if (existing.exists()) throw new Error(`That ${CARDS[cleanCard.card].label} slot has already been used.`);
  ...
  async seedFixtures(fixtures: FixtureSeed[]): Promise<number> {
    ...
    for (const fixture of missing) {
      ...
      await setDoc(doc(matchesCol, match.id), match);
      if (!this.state.matches.some(m => m.id === match.id)) this.state.matches.push(match);
      written++;
    }
    this._onUpdate?.();
    return written;
